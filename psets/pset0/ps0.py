...
# Runtime: O(n)
def calculate_sizes(v: BTvertex | None) -> int:
    if v is None:
        return 0
...
def FindDescendantOfSize(t: int, v: BTvertex) -> BTvertex:
    assert v is not None and v.size >= t
    if v.size <= 2 * t - 1:
        return v
    if v.left is None or v.left.size < t:
